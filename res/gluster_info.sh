...
### $all_active_tasks            Array of strings           Each array element represents a volume.   The strings within contain active_tasks where each word is seperated with "---" and each element within is seperated by ",,,"
        echo "=(${all_unhealthy_bricks[@]})"
        echo "=(${all_unhealthy_processes[@]})"
        echo "=(${all_errors_warnings[@]})"
        echo "=(${all_active_tasks[@]})"
get_gluster_info() {
    ...
    if [ -n "$gluster_installed" ]; then
        ...
        for volume in "${gluster_volumes[@]}"; do
            ...
            unhealthy_bricks_string=$(echo "${unhealthy_bricks[*]}" | tr -d '[:space:]')
            ...
            unhealthy_processes_string=$(echo "${unhealthy_processes[*]}" | tr -d '[:space:]')
  ...
  # Get gluster info and evaluate the result.
    ...
    if [ $is_gluster_installed = "true" ]; then
        ...
        for ((i=0; i<$number_of_volumes; i++)); do
            ...
            if [ -z "${all_unhealthy_bricks[$i]}" ]; then
            ...
            if [ -z "${all_errors_warnings[$i]}" ]; then
                printf "%-${output_tab_space}s: %s\n" "Errors/ Warnings" "There are no errors/warnings."
            else
            ...
            if [ -z "${all_active_tasks[$i]}" ]; then
                printf "%-${output_tab_space}s: %s\n" "Active Tasks" "There are no active tasks."
            else
